package com.arafet.anime.security;
...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	/*
		...
		*/
		...
		auth.inMemoryAuthentication().withUser("arafet").password(passwordEncoder.encode("123")).roles("AGENT","USER");
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.authorizeRequests().antMatchers("/AjouterAnime").hasAnyRole("ADMIN","AGENT");
		http.authorizeRequests().antMatchers("/saveAnime").hasAnyRole("ADMIN","AGENT");
		http.authorizeRequests().antMatchers("/ListeAnime")
		.hasAnyRole("ADMIN","AGENT","USER");
		http.authorizeRequests()
		.antMatchers("/supprimerAnime","/modifierAnime","/updateAnime")
		.hasAnyRole("ADMIN");
